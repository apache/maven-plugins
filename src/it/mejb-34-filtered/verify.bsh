...
try
{
    File file = new File( basedir, "target/mejb-34-filtered-1.0.jar" );
    ...
    else
    {
        ...
        if ( index >= 0 )
        {
            System.out.println( "FAILURE! The key 'myKey' has not been replaced during filtering" );
            return false;
        }
        index = contents.indexOf( "myValue" );
        if ( index < 0 )
        {
            System.out.println( "FAILURE! The value 'myValue' has not been injected during filtering" );
            return false;
        }
        is.close();
    }
